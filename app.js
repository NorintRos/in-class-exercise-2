  ...
  engine({
    ...
    helpers: {
      ...
      toFixed: (num, decimals) =>
        (parseFloat(num) || 0).toFixed(
          typeof decimals === "number" ? decimals : 2,
        ),
    },
  }),
);
...
app
  ...
  .on("error", (err) => {
    if (err.code === "EADDRINUSE") {
      ...
      console.error(
        `   Either stop the other app using that port, or start with a different one, e.g.:`,
      );
      console.error(`     PORT=3001 npm start   (Git Bash)`);
      console.error(`     $env:PORT=3001; npm start   (PowerShell)`);
      console.error(
        `   To find the PID on Windows: cmd.exe //c "netstat -ano | findstr :${PORT}"\n`,
      );
      process.exit(1);
    }
    throw err;
